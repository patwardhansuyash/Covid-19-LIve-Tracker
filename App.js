...
window.addEventListener('beforeinstallprompt',(e)=>{
  console.log('before install condition')
  e.preventDefault();
  defprompt=e;
  return false;
...
window.onscroll=()=>{
  ...
  if(window.pageYOffset>100){
    homie.innerHTML=`<a id="gotop" href="#lights"><i class="fas fa-angle-double-up fa-2x"></i></a>`
...
burger.addEventListener('click',(e)=>{
  ...
  if(defprompt){
    defprompt.prompt();
    defprompt.userChoice.then((choiceResults)=>{
      if(choiceResults==='dismiss'){
        console.log('user dimsissed installing app')
      }
      else{
